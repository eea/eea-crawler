...
def normalize_marine(doc, config):
    ...
    if "Shipping and Ports Measure" in normalized_doc["objectProvides"]:
        ...
        for extra_field in wm_spm_extra_fields:
            normalized_doc[f'wm_spm_{extra_field}'] = doc["raw_value"].get(
                extra_field)

    if type(doc["raw_value"].get("legislative_reference")) is list:
        normalized_doc['legislative_reference'] = [
            val.get('title') for val in doc["raw_value"]["legislative_reference"]]
    if type(doc["raw_value"].get("theme")) is list:
        normalized_doc['wm_theme'] = doc["raw_value"].get("theme")

    normalized_doc["wm_dpsir_type"] = simplify_list(
        doc.get("raw_value", {}).get("dpsir_type", []))

    if 'Indicator' in normalized_doc["objectProvides"]:
        normalized_doc["description"] = doc["raw_value"]["title"]
        normalized_doc["dpsir"] = simplify_list(
            doc.get("raw_value", {}).get("dpsir_type", []))
