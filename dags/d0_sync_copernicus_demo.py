...
app_config = Variable.get("app_copernicus_demo", deserialize_json=True)
SCHEDULE_INTERVAL = app_config.get("schedule_interval", "@daily")
default_args = {"owner": "airflow"}

TASK_PARAMS = {"params": {"app": "copernicus_demo", "enable_prepare_docs": True}}


@task
def trigger_sync(ignore_delete_threshold):
    app_id = get_app_identifier("copernicus_demo")
...
def d0_sync_copernicus_demo(ignore_delete_threshold=False):
    trigger_sync(ignore_delete_threshold)


sync_copernicus_demo_dag = d0_sync_copernicus_demo()