...
app_config = Variable.get("app_fise_prod", deserialize_json=True)
SCHEDULE_INTERVAL = app_config.get("schedule_interval", "@daily")
default_args = {"owner": "airflow"}

TASK_PARAMS = {"params": {"app": "fise_prod", "enable_prepare_docs": True}}
# TASK_PARAMS = {"params": {"app": "gs", "enable_prepare_docs": True}}


@task
def trigger_sync(ignore_delete_threshold):
    app_id = get_app_identifier("fise_prod")
...
def d0_sync_fise_prod(ignore_delete_threshold=False):
    trigger_sync(ignore_delete_threshold)


sync_fise_prod_dag = d0_sync_fise_prod()
